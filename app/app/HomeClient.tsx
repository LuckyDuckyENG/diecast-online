...
import type { HomeCar, HomeDriver, HomeStats } from '@/lib/homeData';

/**
 * The home page's interactive shell.
 *
 * Data arrives as props from the server component rather than being fetched in
 * effects, so the served HTML contains the driver and model links. Previously a
 * crawler saw empty containers and could not reach a single content page from
 * the front door.
 */
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '14px' }}>
          {drivers.map((driver, i) => (
            <Link
              key={i}
              /*
               * The hub page, not `/browse?driver=`. Query-string filters are
               * unindexable, which is why /drivers/[slug] exists — the home page
               * was still pointing at the pattern those hubs replaced. Falls back
               * to the filter when a driver has too few cars for a hub.
               */
              href={driver.slug ? `/drivers/${driver.slug}` : `/browse?driver=${encodeURIComponent(driver.name)}`}
              style={{
                position: 'relative',
                display: 'block',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div data-reveal="row1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: '18px', ...getRevealStyles('row1'), transition: 'opacity 0.7s ease 0s, transform 0.7s ease 0s' }}>
            {latestCars.slice(0, 4).map((car) => (
              <Link
                key={car.id}
                /* The slug, not the UUID. Routing accepts both, but the
                   sitemap and canonicals use slugs, so linking by id points the
                   front page at a second URL for the same content. */
              ...
              >
                ...
                <span style={{ display: 'block', padding: '16px 18px 18px' }}>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '12px', borderTop: '1px solid #ecebe6' }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 600, fontSize: '14px', color: '#1a1916' }}>{car.lowestPrice !== null ? `from $${car.lowestPrice.toFixed(2)}` : 'Check prices'}</span>
                    <span style={{ fontSize: '12.5px', color: '#9a958c' }}>{car.retailers} {car.retailers === 1 ? 'retailer' : 'retailers'}</span>
                  </span>
                </span>
              </Link>
            ))}
